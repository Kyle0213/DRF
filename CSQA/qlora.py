...
save = "llama-2e"
ch2l = load_dataset("json", data_files="./csqa_flattened.json")
...
def preprocess_function1(examples):
    ...
    for question, choice_A,choice_B,choice_C,choice_D,choice_E, thought, answerKey in zip(
        examples['stem'], examples['choice_A'],examples['choice_B'],examples['choice_C'],examples['choice_D'],examples['choice_E'], examples['qc2a_[Thought]'],examples['answerKey']
    ):
        prompt = f"""
    ...
    Please give me your reply as the format as:
        [Thought]:
        [Choice]: (You should only give A or B or C or D or E here)
...
def preprocess_function2(examples):
    ...
    for question, choice_A,choice_B,choice_C,choice_D,choice_E, anwser, reason,qca2m_Thought,qca2m_Method,qca2m_Concept_A,qca2m_Concept_B in zip(
        examples['stem'], examples['choice_A'],examples['choice_B'],examples['choice_C'],examples['choice_D'],examples['choice_E'], examples['answerKey'],examples['qc2a_reason'], examples['qca2m_[Thought]'],examples['qca2m_[Method]'],examples['qca2m_[Concept_A]'],examples['qca2m_[Concept_B]']    
        ):
        prompt = f"""
    Please play the role of a Commonsense Question Answering data builder. Now I will give an example, including [Qustion], [Choice], [Answer] and [Reason]. 
    The question's construction methods include the following：
        Spatial: Concept A appears near Concept B 
        Cause & Effect: Concept A causes Concept B 
        Has parts: Concept A contains Concept B as one of its parts 
        Is member of: Concept A belongs to the larger class of Concept B 
        Purpose: Concept A is the purpose of Concept B 
        Social: It is a social convention that Concept A correlates with Concept 
        Activity: Concept A is an activity performed in the context of Concept B
        Definition: Concept A is a definition of Concept B 
        Preconditions: Concept A must hold true in order for Concept B to take place
    [Example]
        [Question]: {question}
        [Choice]: [A]:"{choice_A}" [B]:"{choice_B}" [C]:"{choice_C}" [D]:"{choice_D}" [E]:"{choice_E}"
        [Answer]: {anwser}
        [Reason]: {reason}
    ...
    """
        ...
        [Method]:{qca2m_Method}
        [Concept A]:{qca2m_Concept_A}
        [Concept B]:{qca2m_Concept_B}
        """
        # print(context, label, thought, hypothesis)
        tokenized = combine_and_mask(prompt, target_text)
        input_ids_list.append(tokenized['input_ids'])
...
def preprocess_function3(examples):
    ...
    for choice_A,choice_B,choice_C,choice_D,choice_E, answer, qca2m_Thought,qca2m_Method,qca2m_Concept_A,qca2m_Concept_B,cam2q_Thought,Question in zip(
        examples['choice_A'],examples['choice_B'],examples['choice_C'],examples['choice_D'],examples['choice_E'], examples['answerKey'], examples['qca2m_[Thought]'],examples['qca2m_[Method]'],examples['qca2m_[Concept_A]'],examples['qca2m_[Concept_B]'],examples['cam2q_[Thought]'],examples['cam2q_[Question]'],
    ):
        prompt = f"""
    ...
    [Method] means that the [Question] is based on the category created by [Method_Concept_A] and [Method_Concept_B].
    [Method_Concept_A] and [Method_Concept_B] represent the core concepts used in the construction process of the [Question].
    ...
    [Example]
        ...
        [Method_Concept_A]:{qca2m_Concept_A}
        [Method_Concept_B]:{qca2m_Concept_B}
        [Method]:{qca2m_Method}
